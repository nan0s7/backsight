...
| `reference/collisions.md` | Method for finding and recording naming collisions | Pass 2 |
...
### This file is not one of the working files
...
- **A change that inverts the expected answer.** Renaming the script breaks its ability to find its own running copy, because it identifies itself by filename — silently, with a duplicate process fighting over your fans as the only symptom. It does **not** break the systemd unit, which is the first thing anyone would check, *because that unit was already pointing somewhere else entirely.* Finding nothing to change there reads as confirmation the rename is safe.
...
The map has not been re-run since. That is the honest state.
...
- **The cold-read test passed on the second attempt, not the first.** It failed once, for a reason worth recording: a card asserted that nothing read a folder, the reader went to the territory, and found something that did. The claim had been false since the day it was written — nothing had decayed, the survey simply had not looked. That failure produced the rule about negatives in `rules.md`, and the rewritten card passed. **n=1 on the passing run**, one question, one card. It has not been run across every card or every question shape.
...
- **The evidence base is five territories, four of them the author's own.** The fifth is a third-party repo they had never opened, and it is the one that produced two of the rules. **No third party has operated this** — every walk was run by the author, which is a larger gap than the territory count and is not closed by adding more territories.
